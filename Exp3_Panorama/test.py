...
class TestWarp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        resBl = np.load(r'D:\CVEXP\exp3\Exp3_Panorama\testMat\warpBlank.npy')
...
class TestAlignment(unittest.TestCase):
    ...
    def test_computehomography2(self):
        ...
        A_soln = np.load(r'D:\CVEXP\exp3\Exp3_Panorama\testMat\identityA.npy')
        # print(A_soln)
    ...
    def test_getInliers(self):
        """Tests TODO 5"""
        inliers = alignment.getInliers(self.f1 + [self.outlier_f1], self.f2 + [self.outlier_f2],
                                       self.matches_with_outlier, np.eye(3), 1)
        self.assertTrue(len(inliers) == 4, "Error in getting inliers " + str(len(inliers)))
        inliers = alignment.getInliers(self.f1 + [self.outlier_f1], self.f2 + [self.outlier_f2],
                                       self.matches_with_outlier, np.eye(3), 2)
        self.assertTrue(len(inliers) == 5, "Error in getting inliers")

    def test_leastSquaresFit(self):
        """Tests TODO 6,7"""
        M = alignment.leastSquaresFit(self.f1 + [self.outlier_f1], self.f2 + [self.outlier_f2],
                                      self.matches_with_outlier, 1, [0, 1, 2, 3])
        M = M.astype(float)
        M = M / M[2, 2]
        ...
        M = alignment.leastSquaresFit(self.f1 + [self.outlier_f1], self.f2 + [self.outlier_f2],
                                      self.matches_with_outlier, 1, [0, 1, 2, 4])
        M = M.astype(float)
        M = M / M[2, 2]
...
class TestBlend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rot_trans_transform = np.array(
            [[np.cos(np.pi / 4), -np.sin(np.pi / 4), 5], [np.sin(np.pi / 4), np.cos(np.pi / 4), -5], [0, 0, 1]])
