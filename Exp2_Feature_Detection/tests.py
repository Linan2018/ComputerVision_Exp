    ...
    if type(output) is list or type(output) is tuple:
        for i in range(len(output)):
            ...
            if not compare(output[i], truth[i], **kargs):
                ...
                # print(output[i].angle, truth[i].angle)
                # print(output[i].response, truth[i].response)
                print("TODO {} doesn't pass test: {}".format(todo, i))
                failed += 1
    else:
        if not compare(output, truth, **kargs):

            # for f1, f2 in zip(output, truth):
                # if np.all(f1 != f2) and f1[0] != 0 and f2[0] != 0:
                #     # print("____________________")
                #     # print(f1.reshape(8, 8))
                #     print(f2.reshape(8, 8))
                    # cv2.imshow("o", f1.reshape(8, 8))
                    # cv2.imshow("t", f2.reshape(8, 8))
                    # cv2.waitKey(0)
