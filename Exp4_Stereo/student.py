...
def compute_photometric_stereo_impl(lights, images):
    ...
    for i in range(height):
        for j in range(width):
            ...
            G = (lights.T.dot(lights)).I.dot(lights.T.dot(intensity))

            if channel == 3:
                albedo[i, j, 0] = np.linalg.norm(G[:, 0])
                albedo[i, j, 1] = np.linalg.norm(G[:, 1])
                albedo[i, j, 2] = np.linalg.norm(G[:, 2])
                if albedo[i, j, 0] < 1e-7:
                    albedo[i, j, :] = 0
                    normals[i, j, :] = [0, 0, 0]
                else:
                    normals[i, j, :] = (G[:, 0] / albedo[i, j, 0]).reshape(-1)
            elif channel == 1:
                ...
                if albedo[i, j] < 1e-7:
                    ...
                    normals[i, j, :] = (G / albedo[i, j]).reshape(-1)
...
def project_impl(K, Rt, points):
    ...
    projections = np.array([[fun(i, j) for j in range(width)] for i in range(height)])
...
def form_poisson_equation_impl(height, width, alpha, normals, depth_weight, depth):
    ...
    if depth is not None:
        row = len([1 for j in range(width) for i in range(height) if alpha[i, j] != 0])
        row_ind = list(range(row))
        col_ind = [i * width + j for j in range(width) for i in range(height) if alpha[i, j] != 0]
        data_arr = [depth_weight for j in range(width) for i in range(height) if alpha[i, j] != 0]
        b = [depth_weight * depth[i, j] for j in range(width) for i in range(height) if alpha[i, j] != 0]
